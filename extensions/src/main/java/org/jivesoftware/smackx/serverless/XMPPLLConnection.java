 ...
 *
 */
public class XMPPLLConnection extends TCPConnection /*XMPPConnection*/ // public for debugging reasons
{
     ...
     */
    XMPPLLConnection(LLService service, org.jivesoftware.smackx.serverless.LLConnectionConfiguration config) {
        // TODO: Evaluate whether null CallbackHandler is acceptable
        super(config);
        connection = this;
     ...
     */
    void initListen() throws XMPPException, SmackException {
        initConnection();
    }
     ...
     */
    public void connect() throws XMPPException, SmackException {
    ...
    public void streamInitiatingReceived() throws XMPPException {
        if (config.getServiceName() == null) {
            ...
            setPacketWriter(new PacketWriter(this));
            if (debugger != null) {
                if (debugger.getWriterListener() != null) {
                    addPacketListener(debugger.getWriterListener(), null);
                }
            }
            getPacketWriter().startup();
            notifyLLListenersConnected();
        }
    }
     ...
     */
    private void initConnection() throws SmackException, XMPPException {
        ...
        try {
            ...
            setPacketReader(new LLPacketReader(service, this));
            ...
            getPacketReader().startup();
        }
        // TODO: Ensure change from XMPPException is valid
        catch (SmackException.NoResponseException ex) {
            // An exception occurred in setting up the connection. Make sure we shut down the
            // readers and writers and close the socket.

            if (getPacketWriter() != null) {
                try {
                    getPacketWriter().shutdown();
                }
                catch (Throwable ignore) { /* ignore */ }
                setPacketWriter(null);
            }
            if (getPacketReader() != null) {
                try {
                    getPacketReader().shutdown();
                }
                catch (Throwable ignore) { /* ignore */ }
                setPacketReader(null);
            }
            if (getSocket() != null) {
                try {
                    getSocket().close();
    ...
    private void initReaderAndWriter() throws XMPPException {
        try {
            reader =
                    new BufferedReader(new InputStreamReader(getSocket().getInputStream(), "UTF-8"));
            writer = new BufferedWriter(
                    new OutputStreamWriter(getSocket().getOutputStream(), "UTF-8"));
        }
        catch (IOException ioe) {
    ...
    public void disconnect() {
        ...
        if (getPacketReader() == null || getPacketWriter() == null) {
            return;
        }
